...
Definition End' {C : Cat} (A : Ob C) : Type := {f : Hom A A | True}.
Definition Mon' {C : Cat} (A B : Ob C) : Type := {f : Hom A B | Mon f}.
Definition Epi' {C : Cat} (A B : Ob C) : Type := {f : Hom A B | Epi f}.
Definition Sec' {C : Cat} (A B : Ob C) : Type := {f : Hom A B | Sec f}.
Definition Ret' {C : Cat} (A B : Ob C) : Type := {f : Hom A B | Ret f}.
Definition Iso' {C : Cat} (A B : Ob C) : Type := {f : Hom A B | Iso f}.
Definition Aut' {C : Cat} (A : Ob C) : Type := {f : Hom A A | Aut f}.

Definition Mon'_Hom (C : Cat) (A B : Ob C) (f : Mon' A B) := proj1_sig f.
Definition Epi'_Hom (C : Cat) (A B : Ob C) (f : Epi' A B) := proj1_sig f.
Definition Sec'_Hom (C : Cat) (A B : Ob C) (f : Sec' A B) := proj1_sig f.
Definition Ret'_Hom (C : Cat) (A B : Ob C) (f : Ret' A B) := proj1_sig f.
...
Theorem dual_bim_bim : forall (C : Cat) (A B : Ob C) (f : Hom A B),
...
Theorem dual_iso_iso : forall (C : Cat) (A B : Ob C) (f : Hom A B),
...
Theorem dual_isomorphic : forall (C : Cat) (A B : Ob C),
...
Theorem mon_comp : forall (C' : Cat) (A B C : Ob C') (f : Hom A B)
    (g : Hom B C), Mon f -> Mon g -> Mon (f .> g).
unfold Mon. intros cat A B C f g f_mon g_mon X h1 h2 H.
apply f_mon, g_mon. cat.
Qed.

Theorem epi_comp : forall (C' : Cat) (A B C : Ob C') (f : Hom A B)
    (g : Hom B C), Epi f -> Epi g -> Epi (f .> g).
unfold Epi; intros cat A B C f g f_epi g_epi X h1 h2 H.
apply g_epi, f_epi. cat_simpl'. assumption.
Qed.

Theorem bim_comp : forall (C' : Cat) (A B C : Ob C') (f : Hom A B)
    (g : Hom B C), Bim f -> Bim g -> Bim (f .> g).
unfold Bim; intros. destruct H, H0.
split; [apply mon_comp; assumption | apply epi_comp; assumption].
Qed.

Theorem sec_comp : forall (C' : Cat) (A B C : Ob C') (f : Hom A B)
    (g : Hom B C),
    Sec f -> Sec g -> Sec (f .> g).
